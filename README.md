...
report/assets/           报告中引用的曲线、跟踪帧和分割可视化图片
...
从当前结果看，ImageNet 预训练带来的提升最明确；CBAM 相比 baseline 只提升 0.63 个百分点，应作为低成本潜在增益看待；Swin-T 的 accuracy 最高，但参数量和训练开销也更大。若后续需要补充分类预测样例或错误类别统计，可直接加载已下载的 Task 1 权重，在 Oxford-IIIT Pet 测试集上做推理。
...
报告中额外给出了 counting line 示例和第 252 到 253 帧的局部 ID-switch 放大图。越线计数依赖 Tracking ID 的连续性，因此 38 个目标应理解为当前检测-跟踪流水线的自动统计结果，不是人工核验后的绝对真值。
...
报告中补充了同一批验证图像在 CE、Dice 和 CE+Dice 三个权重下的分割 mask 横向对比图。该图来自已有训练输出和本地权重，不需要重新训练。
...
```text
report/assets/task2_counting_line_example.jpg
report/assets/task2_id_switch_summary.png
report/assets/task2_id_switch_3479_2416.png
report/assets/task2_id_switch_3803_3549.png
report/assets/task3_segmentation_comparison.png
```

这些补充图只使用已有权重、日志、跟踪结果或本地可视化输出，不需要重新训练模型。跟踪帧截图、ID-switch 局部图和补充 PR 曲线可以由已有实验结果重新生成：

```bash
python report/make_assets.py
```

如果是全新环境，请先准备 `results/task2/tracking_final/`、`results/task3/*/visualizations/` 和 `runs/detect/results/task2/visdrone_yolov8/` 中的本地输出；这些目录默认不随 GitHub 提交。
